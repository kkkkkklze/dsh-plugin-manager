...
check('Config 声明 waitMs(防 Schemastery 透传隐患)', indexSrc.includes('waitMs: Schema.number().default(3000)'))
check('FIBER_PHASE 官方语义注释(DISPOSED=4→null)', readFileSync(new URL('./engine.js', import.meta.url), 'utf8').includes('DISPOSED=4'))
...
// 多选整合包并集启用:重复引用只启用一次
const gwMulti0 = await gw.switchPresets([])
check('switchPresets 空数组拒绝', gwMulti0.ok === false, gwMulti0.text)
const gwMultiMiss = await gw.switchPresets(['不存在包'])
check('switchPresets 不存在的包拒绝', gwMultiMiss.ok === false, gwMultiMiss.text)
const gwMulti1 = await gw.addPreset('并集A', 'A', ['tools', 'bash'])
const gwMulti2 = await gw.addPreset('并集B', 'B', ['bash', 'user-row'])
check('addPreset 并集包成功', gwMulti1.ok === true && gwMulti2.ok === true)
const gwMultiSw = await gw.switchPresets(['并集A', '并集B'])
check('switchPresets 并集切换成功', gwMultiSw.ok === true, gwMultiSw.text)
check('并集切换同时启用两包插件', String(gwMultiSw.text).includes('tools') && String(gwMultiSw.text).includes('user-row'), '')
const multiPatch = await fs.readFile(patchPath, 'utf8')
const toolsLines = multiPatch.split('\n').filter((l) => /^\s*disabled: (true|false)/.test(l))
check('重复引用 bash 只写一条启停行', (multiPatch.match(/- id: bash\n/g) || []).length === 1, '')
const gwRmMulti1 = await gw.removePreset('并集A')
const gwRmMulti2 = await gw.removePreset('并集B')
check('清理并集包成功', gwRmMulti1.ok === true && gwRmMulti2.ok === true)
// 切换成功写入 lastSwitch 待确认标记(最近一次成功切换 = 并集)
const stAfterMulti = JSON.parse(await fs.readFile(path.join(tmp, 'plugin-manager', 'state.json'), 'utf8'))
check('并集切换写入 lastSwitch(并集A + 并集B)', stAfterMulti.lastSwitch && stAfterMulti.lastSwitch.preset === '并集A + 并集B', JSON.stringify(stAfterMulti.lastSwitch || null))
...
// 安全:命令注入防护 —— 非法仓库名一律拒绝
const inj1 = await gw.marketInstall('foo/bar; calc')
check('marketInstall 拒绝命令注入(分号)', inj1.ok === false && String(inj1.text).includes('非法'), inj1.text)
const inj2 = await gw.marketInstall('foo/bar && whoami')
check('marketInstall 拒绝命令注入(&&)', inj2.ok === false && String(inj2.text).includes('非法'), inj2.text)
...
// 安全:保留键名与超大输入拒绝
const protoImp = await gw.importPreset('name: __proto__\nplugins: [x]')
check('importPreset 拒绝 __proto__ 预设名', protoImp.ok === false, protoImp.text)
const bigImp = await gw.importPreset('name: big\n' + 'description: ' + 'x'.repeat(300000))
check('importPreset 拒绝超大输入(256KB)', bigImp.ok === false && String(bigImp.text).includes('256KB'), bigImp.text)
const protoAdd = await gw.addPreset('__proto__', 'x', ['tools'])
check('addPreset 拒绝 __proto__ 预设名', protoAdd.ok === false, protoAdd.text)
